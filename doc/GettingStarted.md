...
- on memory-constrained hosts (WSL, small VMs, Docker Desktop, containers with a `--memory` limit), `build.sh` automatically caps build parallelism to prevent OOM — no manual configuration needed; see [Memory Requirements](development/BuildingDaphne.md#memory-requirements) for details and manual override options
- Optional:
