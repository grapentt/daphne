...
- on memory-constrained hosts (WSL, small VMs, Docker Desktop, containers with a `--memory` limit), see [Memory Requirements](development/BuildingDaphne.md#memory-requirements) for how to cap build parallelism and avoid OOM during compilation
- Optional:
