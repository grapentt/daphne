...
The header-vs-.cpp split for distributedPipeline (see the refactor that
introduced this script) removes a large transitive header closure from the
...
surfaces late in the build and is confusing to diagnose.
...
# The (?<![<:\w]) prefix and (?![:\w]) suffix avoid matching partial words
# (e.g. accidentally matching `<opName>_something`).
